		...
		people: {
			_peopleId: (val1: number) => {
				...
				return {
					...
					$get: (option?: { config?: T }) =>
						fetch<Methods0["get"]["resBody"]>(prefix, prefix1, GET, option).json().then(
							(r) => r.body,
						),
					put: (option: { body: Methods0["put"]["reqBody"], config?: T }) =>
						fetch<Methods0["put"]["resBody"]>(prefix, prefix1, PUT, option).json(),
					$put: (option: { body: Methods0["put"]["reqBody"], config?: T }) =>
						fetch<Methods0["put"]["resBody"]>(prefix, prefix1, PUT, option).json().then(
							(r) => r.body,
						),
					$path: () => `${prefix}${prefix1}`,
				};
			...
			users: {
				_userId: (val2: number) => {
					...
					return {
						...
						$get: (option?: { config?: T }) =>
							fetch<Methods2["get"]["resBody"]>(prefix, prefix2, GET, option).json().then(
								(r) => r.body,
							),
						put: (option: { body: Methods2["put"]["reqBody"], config?: T }) =>
							fetch<Methods2["put"]["resBody"]>(prefix, prefix2, PUT, option).json(),
						$put: (option: { body: Methods2["put"]["reqBody"], config?: T }) =>
							fetch<Methods2["put"]["resBody"]>(prefix, prefix2, PUT, option).json().then(
								(r) => r.body,
							),
						$path: () => `${prefix}${prefix2}`,
					};
				...
				$get: (option?: { query?: Methods1["get"]["query"], config?: T }) =>
					fetch<Methods1["get"]["resBody"]>(prefix, PATH1, GET, option).json().then(
						(r) => r.body,
					),
				post: (option: { body: Methods1["post"]["reqBody"], config?: T }) =>
					fetch<Methods1["post"]["resBody"]>(prefix, PATH1, POST, option).json(),
				$post: (option: { body: Methods1["post"]["reqBody"], config?: T }) =>
					fetch<Methods1["post"]["resBody"]>(prefix, PATH1, POST, option).json().then(
						(r) => r.body,
					),
